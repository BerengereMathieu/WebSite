...
selectAreaTools.main=function(){
	...
	if(test.error){
		...
		var error_msg1=$("<div><p>Il semble que votre navigateur ne possède pas les fonctionnalités nécessaires à l'éxecution de cette démo :(</p><img width='256px' src='images/Adoptfirefox.jpg'></div>");
	...
	selectAll.click(function(click){
		if(allSelected){
			...
			for(id in selectAreaTools.selectedLayers){
				...
				selectAreaTools.selectedLayers[id].buttonJQ.css('background-image','url("images/delete-icon.png")');
			}
...
selectAreaTools.addHandlerToInitialImage=function(){
	...
	hideCheckBox.click(function(event){
		if(initialImageHiden){
			...
			//show initial image	
			hideCheckBox.css('background-image','url("images/eye-icon.jpg")');	
...
selectAreaTools.addMiniatureLayer=function(miniature,layerId){
	...
	hideCheckBox.css('background','url("images/eye-icon.jpg")');
	...
	checkedButton.click(function(event){
		if(!selectAreaTools.selectedLayers[layerId].activate){
			checkedButton.css('background-image','url("images/delete-icon.png")');
	...
	commentTool.css('background-image','url("images/stylo.jpg")');	
